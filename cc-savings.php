...
Plugin URI: https://calculatorscanada.ca/savings-calculator-wordpress-widget/
...
class cc_savings extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if ( $allow_cc_urls && !empty($title))
			 $title = '<a href="https://calculatorscanada.ca/savings-interest-calculator/" target="_blank" style="text-decoration:none">' . esc_attr($title) . '</a>';		
...
function cc_savings_init ()
{
    return register_widget('cc_savings');
}
add_action ('widgets_init', 'cc_savings_init');
...
function cc_savings_scripts() {
    ...
    wp_enqueue_script( 'cc-savings', plugins_url('/cc-savings.js',__FILE__), array('jquery'), '1.2.0', true );
}
